...
#                     name=name, overwrite=entry['overwrite'], page=int(
#                         entry['page']),
#                     subscriptions=[IllustsSubscription(
#                         s_type=sub['type'], uid=sub['uid']) for sub in entry['subscriptions']]
#                 )
...
#         if len(work_uids) == 0:
#             work_uids = [
#                 sub.uid for sub in db_subs.subscriptions if sub.s_type == "work"]
...
#             bookmark_uids = [
#                 sub.uid for sub in db_subs.subscriptions if sub.s_type == "bookmark"]
#             if -1 in work_uids and self.papi.account_session is None:
#                 raise RuntimeError("未登录 Pixiv")  # 一般调用此函数时已经登录
#             bookmark_uids = [
