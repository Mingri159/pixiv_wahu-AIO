    ...
    <q-card style="min-width: 400px; width: 50vw">
      ...
      <q-card-section v-if="config !== undefined">
        <q-form @submit="onSubmit">
          ...
          <q-select v-model="config.subscribe_overwrite" :options="['append', 'intelligent', 'replace']" label="覆写模式"
            hide-hint hint="intelligent: 当某一页所有插画均在数据库中，停止更新；append: 追加指定页数的插画；replace: 删除原有插画"></q-select>

          <q-input type="number" v-model="config.subscribe_pages" label="更新订阅的页数"
            v-if="config.subscribe_overwrite != 'intelligent'" hint="-1 表示不限页数" hide-hint></q-input>
...
function reset(name = props.dbName) {
